...
// Build 50 members across 12 teams
const TEAM_SIZES = [5,4,5,4,4,4,4,4,4,4,4,4]; // 50
const MEMBERS = (() => {
  const out = [];
  let id = 1;
...
const ME = MEMBERS.find(m => m.id === 'u15'); // 로그인된 사용자 (디자인팀 가정 X — 임의 선택)
const ADMIN = MEMBERS[0]; // 관리자: 미디어팀 팀장

// ── 일정 데이터 ──────────────────────────────────────────
// 기준 날짜: 2026-04-30 (목)
const TODAY = new Date(2026, 3, 30); // month is 0-indexed
...
// 개인 일정 (휴가/반차/체육대회 참석 등)
// 종일 일정은 startDate~endDate, 반차는 half: 'AM'|'PM'
...
// ── 식단표 ───────────────────────────────────────────────
// 주차별 데이터 - 4/27~5/1, 5/4~5/8, 5/11~5/15
const MEALS = {
  '2026-04-27': [
...
Object.assign(window, {
  TEAMS, MEMBERS, ME, ADMIN, COMPANY_EVENTS, PERSONAL_EVENTS, MEALS, TODAY,
});
